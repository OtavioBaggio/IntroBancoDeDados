...
where Genero.Descricao = "Infantil" and Livro.idGenero = Genero.idGenero and
...
select *
from autor
order by nome desc;	  -- 'desc' é abreviação de "DESCENDING", usado para ordem decrescente
...
select round(avg(preco),2) as preco_Medio_Arrendodado
from livro
where idEditora = 2;
